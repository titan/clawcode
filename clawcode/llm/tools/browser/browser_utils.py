...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-clawcode_…`` the resulting socket path exceeds the
...
def _run_browser_command(
    ...
    try:
        ...
        # Ensure PATH includes ClawCode-managed Node first (if present), then
        # Hermes-managed Node as a migration fallback, then system dirs.
        ...
        fallback_home = Path(os.environ.get("HERMES_HOME", Path.home() / ".hermes"))
...
def _cleanup_old_recordings(max_age_hours=72):
    ...
    try:
        try:
            ...
            data_dir = Path(os.getenv("CLAWCODE_HOME", Path.home() / ".clawcode"))
        recordings_dir = Path(data_dir) / "browser_recordings"
...
# ---------------------------------------------------------------------------
try:
    # Hermes registry (optional). ClawCode wires tools via BaseTool later.
