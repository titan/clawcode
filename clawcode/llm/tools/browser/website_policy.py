...
This module loads a user-managed website blocklist from ClawCode Settings
...
def _get_clawcode_data_dir() -> Path:
    """Resolve ClawCode data directory for relative shared_files.

    When settings aren't available (e.g. imported early), fall back to
    ``$CLAWCODE_HOME`` or ``~/.clawcode``.
    """

    try:
        return get_settings().get_data_directory()
    except Exception:
        return Path(os.getenv("CLAWCODE_HOME", Path.home() / ".clawcode"))


def _get_default_config_path() -> Path:
    return _get_clawcode_data_dir() / "config.yaml"
...
def load_website_blocklist(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if config_path is None:
        ...
        # Settings-based (new ClawCode-local mode): no YAML read.
        try:
    ...
    for shared_file in raw_shared_files:
        ...
        if not path.is_absolute():
            path = (_get_clawcode_data_dir() / path).resolve()
