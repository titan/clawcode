...
def test_action_send_message_builtin_experience_apply_routes_builtin(tmp_path) -> None:
    settings = Settings()
    ...
    input_w = _fake_input("/experience-apply ecap-1")
...
def test_action_send_message_plugin_namespace_experience_uses_dispatch(tmp_path) -> None:
    settings = Settings()
    settings.working_directory = str(tmp_path)
    screen = ChatScreen(settings)
    sid = "sess-plugin-exp-route"
    screen.current_session_id = sid
    screen._agent = MagicMock()
    ...
    assert rs is not None
    rs.is_processing = False
    input_w = _fake_input("/plugin:experience-apply ecap-1")
    ...
    with (
        ...
        patch(
            ...
            return_value=SlashDispatch(consume_without_llm=False, llm_user_text="PLUGIN_EXP_BODY"),
        ),
    ...
    assert kwargs["content_for_agent"] == "PLUGIN_EXP_BODY"
