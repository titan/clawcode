"""End-to-end style checks for /multi-execute routing vs provider toggles and show/list."""
...
@pytest.mark.asyncio
async def test_multi_execute_e2e_disabled_provider_excluded_from_routing_and_artifacts(
    tmp_path: Path,
) -> None:
    ...
    session_id = "sess_e2e_multi_execute"
    ...
    out_all = await handle_builtin_slash(
        "multi-execute",
        "ship retry logic --explain-routing --strategy balanced --audit on",
        settings=settings,
    ...
    assert meta_all.get("execution_meta", {}).get("audit") == "on"
    ...
    out_quality = await handle_builtin_slash(
        "multi-execute",
        "ship retry logic --explain-routing --strategy quality-first --audit on",
        settings=settings,
        session_service=None,
    )
    ...
    bundle = store.save_bundle_versioned(
        ...
        user_request="ship retry logic",
        plan_text="# Multi-Execute Result: e2e\n\n- step",
        tasks=[PlanTaskItem(id="task-e2e-1", title="step")],
