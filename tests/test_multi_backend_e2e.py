"""End-to-end style checks for /multi-backend routing vs provider toggles and show/list."""
...
@pytest.mark.asyncio
async def test_multi_backend_e2e_disabled_provider_excluded_from_routing_and_artifacts(
    tmp_path: Path,
) -> None:
    settings = Settings()
    settings.working_directory = str(tmp_path)
    session_id = "sess_e2e_multi_backend"
    ...
    out_all = await handle_builtin_slash(
        "multi-backend",
        "retry idempotency --explain-routing --strategy balanced --audit on",
        settings=settings,
    ...
    assert meta_all.get("workflow") == "backend"
    assert meta_all.get("backend_meta", {}).get("audit") == "on"
    auth_chain = [c["model_id"] for c in meta_all["candidate_chains"]["backend_authority"]]
    assert "deepseek-chat" in auth_chain
    ...
    auth_chain2 = [c["model_id"] for c in meta_cut["candidate_chains"]["backend_authority"]]
    ...
    bundle = store.save_bundle_versioned(
        ...
        user_request="retry idempotency",
        plan_text="# Multi-Backend Result: e2e\n\n- step",
        tasks=[PlanTaskItem(id="task-mb-e2e", title="step")],
        subdir="multi-backend",
        base_name="e2e-retry",
    )
    bundle.routing_meta = dict(meta_cut)
    bundle.routing_meta["backend_meta"] = {"audit": "on"}
